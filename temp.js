...
function renderDashboardKPIs(filteredMaster, filteredTests) {
  ...
  fetchAndRenderEngineeringCharts();
}
...
<!-- HIDDEN CUBE TEST REPORT TEMPLATE (FOR PDF EXPORT VIA html2pdf.js) -->
<div id="cube-test-report-template" style="display: none;">
...
<script>
/* ---- Mobile: resize / orientation change handler ---- */
(function() {
  ...
  window.addEventListener('orientationchange', function() {
    setTimeout(onLayoutChange, 300);
  });

  // Tap sidebar backdrop to close drawer
  document.addEventListener('click', function(e) {
    const sb = document.getElementById('sidebar');
    const topbar = document.getElementById('mobile-topbar');
    if (sb && sb.classList.contains('show') && topbar) {
      // If click target is the sidebar itself (not a child), close it
      if (e.target === sb) {
